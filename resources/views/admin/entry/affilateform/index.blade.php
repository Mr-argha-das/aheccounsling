...
<div class="block p-3"  style="border-radius:15px">
    <div class="table table-vcenter table-hover table-responsive">
        <table class="myTable">
            ...
            <td>
                    @if($ob->af_file!='')
                  @foreach(json_decode($ob->af_file)  as $key => $file)

                   <a href="{{ asset($ob->folder_path.$file) }}" target="_blank" class="btn btn-primary btn-sm px-2 py-1 mb-2">Download File {{ $key+1 }}</a>
