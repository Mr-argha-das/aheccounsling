...
<div class="block" style="border-radius:15px">
    ...
    <?=Design::$filterStart?>
      <div class="col-md-6">
         {{ Form::bsText('blog_title','GET_METHOD',['label'=>"Blog Title"]) }}
      </div>
      <?=Design::$filterClose?>
 <div class="block">
            ...
            <tbody>
                ...
                ?>
                    <tr>
                        ...
                        <td>{{ $ob->blog_name }}</td>
                         <td>{{ $blogType[$ob->blog_type] }}</td>
               
                        <td><?php if(!empty($ob->blog_image)){ ?>
                            <img src="{{  asset('assets/uploads/blogs/'.$ob->blog_image) }} " width="60" width="60">
                            <?php } ?></td>

                         
                         <td><?=($ob->blog_status == 1)?'Admin':$ob->user_name?></td>
                         <td>{{ $statusDropdown[$ob->blog_status] }}</td>
