...
<div class="row">
        ...
        <div class="col-md-6" >
           ...
           <select   id="client_id" name="client_id" class="form-control form-control-lg"> 
               ...
               <option  <?php if($client->user_id==$editData->client_id){ echo 'selected'; }   ?> value="{{$client->user_id}}">{{$client->user_name}}  ({{$client->user_password}})</option>
               @endforeach
          </select> 
       </div>
         <div class="col-md-6">
               <label class="control-label">Service Type</label>
                <select  name="service_id" id="service_id"  class="form-control form-control-lg"> 
                      <optionvalue="">Service Type</option>
                        @foreach($serviceArray as $key =>$vs)
                        <option <?php if($key==$editData->productinfo){ echo 'selected'; }   ?>  value="{{ $key }}">{{ $vs}} </option>
                        @endforeach
                   </select>
          </div>
          <div class="col-md-6">
               <label class="control-label">Payment Currency Type</label>
                <select  name="currenct_type" id="currenct_type"  class="form-control form-control-lg"> 
                      <option value="">Currency Type</option>
                        ...
                        <option value="{{ $paymentCurrencyType->id }}"  <?php if($paymentCurrencyType->id==$editData->payment_key_type){ echo 'selected'; }   ?>>{{ $paymentCurrencyType->type }} </option>
                        @endforeach
               </select>
           </div>
         ...
         <div class="col-md-6">
           ...
           <input type="text" name="amount" class="form-control form-control-lg numbers" id="amount"  value="{{$editData->amount}}"  placeholder="Amount">
        </div>
